# 6. Да се състави програма за обработка на масива A[N,N], където данните са цели числа в
# интервала [-500;500]. Програмата да извърши следните действия:
...
# елементи от всяка колона на масива А;
# ● б) полученият масив да се сортира по големина;
# ● отпечатване на получените резултати след обработка а) и след обработка б)

def print_condition():
    print("\nЗадача: Обработка на двумерен масив A[N,N] с цели числа в интервала [-500;500].\n"
          "1. Да се образува едномерен масив C с максималните елементи от всяка колона на масива A.\n"
            ...
            while True:
                try:
                    ...
                    if -500 <= element <= 500:
                        ...
                        print("Моля, въведете число в интервала [-500, 500].")
                except ValueError:
...
def find_column_max_elements(matrix):
    n = len(matrix)
    c = [max(matrix[i][j] for i in range(n)) for j in range(n)]
    return c
...
def main():
    ...
    c = find_column_max_elements(a)
    print(f"\nМасив C с максималните елементи от всяка колона: {c}")
